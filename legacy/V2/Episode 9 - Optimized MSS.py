...
class App(CTk):
    ...
    def build_basic_tab(self, parent):
        ...
        # Gift box color and tolerance
        CTkLabel(color_settings, text="Gift Box:").grid(row=7, column=0, padx=12, pady=10, sticky="w")
        gift_box_color_var = StringVar(value="#00990c")
        self.vars["gift_box_color"] = gift_box_color_var
        CTkEntry(color_settings, width=120, textvariable=gift_box_color_var).grid(row=7, column=1, padx=12, pady=10, sticky="w")
        ...
        CTkEntry(color_settings, width=120, textvariable=gift_box_tolerance_var).grid(row=7, column=3, padx=12, pady=10, sticky="w")
    def build_automation_tab(self, parent):
        ...
        CTkLabel(toggles, text="Casting Mode:").grid(row=3, column=0, padx=12, pady=10, sticky="w" )
        ...
        casting_cb.grid(row=3, column=1, padx=12, pady=10, sticky="w")
        self.comboboxes["casting_mode"] = casting_cb
        CTkLabel(toggles, text="Shake Mode:").grid(row=5, column=0, padx=12, pady=10, sticky="w" )
        ...
        shake_cb.grid(row=5, column=1, padx=12, pady=10, sticky="w")
        ...
        CTkLabel(ratio_settings, text="Ratio Before Tracking:").grid(row=3, column=0, padx=12, pady=10, sticky="w")
        gift_track_ratio_var = StringVar(value="0.05")
        self.vars["gift_track_ratio"] = gift_track_ratio_var
        CTkEntry(ratio_settings, width=120, textvariable=gift_track_ratio_var).grid(row=3, column=1, padx=12, pady=10, sticky="w")
        ...
        CTkLabel(pid_settings, text="Unstable KP:").grid(row=1, column=2, padx=12, pady=10, sticky="w")
        p_gain_var = StringVar(value="0.9")
        self.vars["proportional2_gain"] = p_gain_var
        CTkEntry(pid_settings, width=120, textvariable=p_gain_var).grid(row=1, column=3, padx=12, pady=10, sticky="w")

        CTkLabel(pid_settings, text="Unstable KD:").grid(row=2, column=2, padx=12, pady=10, sticky="w")
        d_gain_var = StringVar(value="0.2")
        self.vars["derivative2_gain"] = d_gain_var
        CTkEntry(pid_settings, width=120, textvariable=d_gain_var).grid(row=2, column=3, padx=12, pady=10, sticky="w")

        CTkLabel(pid_settings, text="Unstable KI:").grid(row=3, column=2, padx=12, pady=10, sticky="w")
        i_gain_var = StringVar(value="0.15")
        self.vars["integral2_gain"] = i_gain_var
        CTkEntry(pid_settings, width=120, textvariable=i_gain_var).grid(row=3, column=3, padx=12, pady=10, sticky="w")

        CTkLabel(pid_settings, text="Unstable Clamp:").grid(row=4, column=2, padx=12, pady=10, sticky="w")
        pid_clamp2_var = StringVar(value="100")
        self.vars["pid_clamp2"] = pid_clamp2_var
        CTkEntry(pid_settings, width=120, textvariable=pid_clamp2_var).grid(row=4, column=3, padx=12, pady=10, sticky="w")
    ...
    def _get_pid_gains(self, inside_bar=False):
        ...
        try:
            if inside_bar == True:
                ...
                kp = float(self.vars["proportional2_gain"].get() or 0.6)
                kd = float(self.vars["derivative2_gain"].get() or 0.2)
        except:
            kp = 0.6
            kd = 0.2
        return kp, kd
    
    def _pid_control_strict(self, error, bar_center_x=None, inside_bar=False):
        """
        ...
        kp, kd = self._get_pid_gains(inside_bar)
    ...
    def _pid_control(self, error, inside_bar=False):
        now = time.perf_counter()
        ...
        kp, kd = self._get_pid_gains(inside_bar)
        ki = float(self.vars["integral2_gain"].get() or 0.15)
        pd_clamp = float(self.vars["pid_clamp2"].get() or 100)

        # Integral (anti-windup)
        self.pid_integral += error * dt
        self.pid_integral = max(-pd_clamp, min(pd_clamp, self.pid_integral))
        ...
        while self.macro_running: # Main macro loop
            ...
            if controller_mode == 0 and bar_center is not None:
                ...
                if bar_left_screen <= fish_x <= bar_right_screen:
                    control = self._pid_control_strict(error, bar_center, bars_found)
                else:
                    control = self._pid_control(error, bars_found)
